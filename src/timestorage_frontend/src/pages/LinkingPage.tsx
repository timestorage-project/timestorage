...
const LinkingPage: FC = () => {
  ...
  const handleUnlink = async (position: ProjectPosition) => {
    try {
      ...
      // Update local state instead of refetching all data
      setPositions(prevPositions =>
        prevPositions.map(pos => (pos.id === position.id ? { ...pos, qrTagId: null, qrTag: undefined } : pos))
      )
                ...
                <div key={position.id} className='card bg-base-100 shadow-lg'>
                  <div className='card-body'>
                    <h3 className='card-title text-lg'>
                      {t('LINKING_POSITION_NUMBER')} {position.positionNumber}
                    </h3>
                    <div className='space-y-1 text-sm mb-4'>
                      <p>
                        <strong>{t('LINKING_DESCRIPTION')}:</strong> {position.description}
                      </p>
                      {position.buildingFloor && (
                        <p>
                          <strong>{t('LINKING_FLOOR')}:</strong> {position.buildingFloor}
                        </p>
                      )}
                      {position.assetModel && (
                        <p>
                          <strong>{t('LINKING_ASSET')}:</strong> {position.assetModel.brand} {position.assetModel.model}
                        </p>
                      )}
                    </div>
                    ...
                    <div className='card-actions justify-end mt-4'>
                      <button
                        ...
                        onClick={() => navigate(`/view/${position.qrTag?.serialNo}`)}
                        disabled={!position.qrTag?.serialNo}
                      >
                        {t('LINKING_MANAGE')}
                      </button>
                      <button
